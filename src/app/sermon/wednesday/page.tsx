...
function WednesdaySermonPageContent() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      setIsLoading(true)
      try {
        const response = await getBoardList(BoardCategory.WEDNESDAY_WORSHIP, currentPage, 9)
        if (response.data) {
  ...
  return (
    <div className="flex flex-col min-h-screen">
      ...
      <main className="flex-grow">
        <SermonPageHeader title="주보" subtitle="Bulletin" />
...
export default function WednesdaySermonPage() {
  return (
    <Suspense fallback={<div>Loading...</div>}>
      <WednesdaySermonPageContent />
    </Suspense>
  )
}
